...
use crate::common::api::logger::infra::{LogFilter, LogLevel, LogMessage, LogOutputLevel, Logger};

pub struct JsonLogger<R: std::fmt::Debug + Serialize + Clone> {
    level: LogOutputLevel,
    request: R,
}

impl<R: std::fmt::Debug + Serialize + Clone> JsonLogger<R> {
    pub fn new(level: LogOutputLevel, request: R) -> Self {
        Self { level, request }
    }

    fn message(&self, level: LogLevel, message: &impl LogMessage) -> String {
        LogEntry::with_message(level.as_str(), message, self.request.clone()).to_json()
    }
}

impl<R: std::fmt::Debug + Serialize + Clone> Logger for JsonLogger<R> {
    fn log(&self, message: &(impl LogFilter + LogMessage)) {
        match self.level {
            LogOutputLevel::Verbose => {
                println!("{}", self.message(message.log_level(), message))
            }
            LogOutputLevel::Info => match message.log_level() {
                LogLevel::Debug => (),
                level => {
                    println!("{}", self.message(level, message))
                }
            },
            LogOutputLevel::Quiet => match message.log_level() {
                LogLevel::Debug => (),
                LogLevel::Info => (),
                level => {
                    println!("{}", self.message(level, message))
...
impl<R: std::fmt::Debug + Serialize + Clone> Logger for QuietLogger<R> {
    fn log(&self, message: &(impl LogFilter + LogMessage)) {
        match message.log_level() {
            ...
            LogLevel::Important => println!("{}", self.message(IMPORTANT, message)),
            LogLevel::Info => (),
            LogLevel::Debug => (),
        }
    }
