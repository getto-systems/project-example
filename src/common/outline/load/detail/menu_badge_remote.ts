...
import {
    ...
    remoteCommonError,
    remoteInfraError,
} from "../../../util/remote/detail/helper"
import { decodeProtobuf } from "../../../util/protobuf/helper"
...
async function fetchRemote(): Promise<LoadMenuBadgeRemoteResult> {
    try {
        ...
        const message = decodeProtobuf(
            pb.common.outline.load.service.LoadOutlineMenuBadgeResponsePb,
            await response.text(),
        )
        return {
            success: true,
...
function decodeItems(
    items: readonly pb.common.outline.load.service.IMenuBadgePb[],
