...
pub struct TonicNotifyUnexpectedErrorProxyCall<'a> {
    service: GoogleTonicService<'a>,
}

impl<'a> TonicNotifyUnexpectedErrorProxyCall<'a> {
...
impl<'a> ProxyCall for TonicNotifyUnexpectedErrorProxyCall<'a> {
    type Request = String;
    ...
    async fn call(
        ...
        let response = client.notify(request).await.map_err(CoreProxyError::from)?;
